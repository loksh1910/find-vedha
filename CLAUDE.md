...
| `src/components/providers/presence-provider.tsx` | `usePresence()` — one app-wide `presence:global` channel (root layout, keyed by uid). Status **derived from the route** (`/room/X` → in-lobby, `/room/X/play` → in-game, else online), re-`track()`ed on nav. Also holds `user:<uid>` for invite (`event:"invite"`) + friend-request (`event:"notify"`) nudges. Exposes `statusOf`, `myLobbyCode`, `invite`/`inviteToCode`, `invites`, and the **friend-request** state (`friendRequests`, `respondFriendRequest`, `sendFriendRequest`, `refreshFriendRequests`, `notifCount`). `presence-pill.tsx` / `invite-toasts.tsx` / `notification-bell.tsx` render it. Friend requests come from the `friendships` RPCs; presence + invites are pure Realtime. |
| `src/components/shell/notification-bell.tsx` | bell + count badge in the dashboard header; click-away popover of invites (Join) + friend requests (Accept/Decline). The Friends page reads the same provider state, so they stay in sync. |
...
- `/friends` — add by username, incoming requests (accept/decline), friends list (Remove) with a **live presence pill** (Online / In a lobby · CODE / In a game), "recent players" you've been in a match with (one-click Add). Backed by `friendships` + the `*_friend*` RPCs. An **Invite** button appears on a friend row when *you* are in a lobby → sends a broadcast to their `user:<uid>` channel → they get an invite toast with **Join**.
