...
| `src/components/providers/presence-provider.tsx` | `usePresence()` — one app-wide `presence:global` channel (root layout, keyed by uid). Status is **derived from the route** (`/room/X` → in-lobby, `/room/X/play` → in-game, else online), re-`track()`ed on nav. Also subscribes `user:<uid>` for invite broadcasts. `statusOf(uid)`, `myLobbyCode`, `invite(uid)`, `invites`. `presence-pill.tsx` + `invite-toasts.tsx` render it. No DB — pure Realtime. |
